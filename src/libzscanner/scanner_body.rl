		...
		if (s->comma_list) {
			...
			if (last_two[1] == ',') {
				if (current_len <= 1) {
					WARN(ZS_EMPTY_LIST_ITEM);
					fhold; fgoto err_line;
				} else if (last_two[0] != '\\' || !s->pending_slash) { // Start a new item.
				...
				if (s->pending_slash) { // Remove backslash.
					rdata_tail--;
					s->pending_slash = false;
				} else {
					s->pending_slash = true;
				}
			} else if (s->pending_slash) {
