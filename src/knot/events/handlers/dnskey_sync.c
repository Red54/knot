...
{
	char buf[512], type_buf[16] = { 0 }, bracket = 0, rem_part = 0;
	...
	for (int i = 0; i < REMOTE_NTYPES; i++) {
		...
		wire_ctx_printf(&w, "%ss: +%hu/-%hu ", type_buf, data->add_rr[i]->rrs.count, data->rem_rr[i]->rrs.count);
		if (remote_rrs[i] == KNOT_RRTYPE_DNSKEY) {
			...
			for (int j = 0; !rem_part || j < data->rem_rr[i]->rrs.count; j++) {
				...
				uint16_t keytag;
				const dnssec_binary_t bin = { .size = dnskey->len, .data = dnskey->data };
				if (dnssec_keytag(&bin, &keytag) != 0) {
					continue;
				}
				wire_ctx_printf(&w, "%c%c%d", bracket ? ' ' : '(', rem_part ? '-' : '+', keytag);
				bracket = 1;
	...
	if (w.error == KNOT_EOK) {
		log_zone_info(data->zone->name, "DNSKEY sync, %s", buf); // intentionally not DNSKEY_SYNC_LOG to save space on log line
	}
}
